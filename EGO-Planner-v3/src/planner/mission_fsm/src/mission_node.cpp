...
ros::Subscriber goal_set_sub_, goal_vla_sub_;

void goalSetCallback(const quadrotor_msgs::GoalSet::ConstPtr& msg);
void uavActionsCallback(const geometry_msgs::PoseStamped::ConstPtr& msg);
...
int main(int argc, char **argv){
  ...
  goal_vla_sub_ = nh.subscribe("/uav_actions", 2, &uavActionsCallback,
                                              ros::TransportHints().tcpNoDelay(true));
...
void uavActionsCallback(const geometry_msgs::PoseStamped::ConstPtr& msg){
  INFO_MSG_BLUE("[MAIN]: ===== Received new goal from VLA =====");
  pubLocalGoal(Eigen::Vector3d(msg->pose.position.x, msg->pose.position.y, msg->pose.position.z), 0.0, true);
}
...
void pubLocalGoal(Eigen::Vector3d local_aim_pose, double yaw_aim, bool look_forward){
  ...
  ego_goal_msg.goal[2]  = local_aim_pose.z(); 
