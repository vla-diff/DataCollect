...
RCCtrlFSM::RCCtrlFSM(ros::NodeHandle &node, MapInterface::Ptr & map_interface) {
  ...
  data_.reset(new RCCtrlFSMData());
  params_.reset(new RCCtrlFSMParams());
  ...
  readParam(node, "rc_ctrl/loop_rate", params_->fsm_exec_freq_, 30.0);
  ...
  fsm_vis_timer_    = node.createTimer(ros::Duration(1.0 / 30.0), &RCCtrlFSM::fsmVisTimerCallback, this);
  odom_sub_         = node.subscribe<nav_msgs::Odometry>(params_->odom_sub_topic_, 1, boost::bind(&Odom_Data_t::feed, &data_->odom_data_, _1),
                                                 ros::VoidConstPtr(), ros::TransportHints().tcpNoDelay(true));
  rc_sub_           = node.subscribe<mavros_msgs::RCIn>(params_->rc_sub_topic_, 1, boost::bind(&RC_Data_t::feed, &data_->rc_data_, _1),
                                                ros::VoidConstPtr(), ros::TransportHints().tcpNoDelay(true));
  px4_state_sub_    = node.subscribe<mavros_msgs::State>("/mavros/state", 1, boost::bind(&RC_Data_t::feedMavrosState, &data_->rc_data_, _1),
                                                 ros::VoidConstPtr(), ros::TransportHints().tcpNoDelay(true));

  px4ctrl_cmd_pub_          = node.advertise<quadrotor_msgs::PositionCommand>(params_->px4ctrl_cmd_topic_, 10);
  ...
  uav_actions_pub_          = node.advertise<geometry_msgs::PoseStamped>("/uav_actions", 10);
  ...
  data_->last_cmd_pub_time_ = ros::Time(0);
...
void RCCtrlFSM::fsmProcess() {
  ...
  switch (data_->fsm_state_) {
    ...
    case WAIT_FOR_RC_FLAG:
      ...
      data_->expected_pos_ = data_->odom_data_.p_ / M_PI * 180.0;
    ...
    case EXECUTE_RC_CTRL:
      if (!data_->rc_data_.activate_rc_ctrl_mode_)
        changeFSMState("rc ctrl mode dis-activate", RCCtrlFSMState::WAIT_FOR_RC_FLAG);
      setHoverWithRC();
      break;
...
void RCCtrlFSM::setHoverWithRC() {
  ...
  data_->expected_speed_[0] = min((ch_tmp[1] * cos(yaw) + ch_tmp[0] * sin(yaw)), 1.0) * params_->max_speed_;
  data_->expected_speed_[1] = min((ch_tmp[1] * sin(yaw) - ch_tmp[0] * cos(yaw)), 1.0) * params_->max_speed_;
  data_->expected_speed_[2] = ch_tmp[2] * params_->max_speed_;
  data_->expected_yaw_rate_ = ch_tmp[3] * params_->max_yaw_rate_;

  if (!checkMovementSafety(1.0, data_->expected_speed_)){
    holdPosition(data_->odom_data_.p_, data_->odom_data_.yaw_);
...
void RCCtrlFSM::holdPosition(const Eigen::Vector3d &pos, const double &yaw) {
  quadrotor_msgs::PositionCommand cmd_msg;
...
bool RCCtrlFSM::findAvailablePointInRange(Eigen::Vector3d &pos_refine, const double &range, const double & step_size) {
  ...
  while (!q.empty()){
    ...
    if (map_interface_->getInflateOccupancy(last_pos) == MapInterface::OCCUPANCY::FREE){
      ...
      // ========== 新增：发布到 /uav_actions ==========
      geometry_msgs::PoseStamped uav_action_msg;
      uav_action_msg.header.stamp = ros::Time::now();
      uav_action_msg.header.frame_id = "world";
      uav_action_msg.pose.position.x = pos_refine[0];
      uav_action_msg.pose.position.y = pos_refine[1];
      uav_action_msg.pose.position.z = pos_refine[2];
      
      uav_action_msg.pose.orientation.x = data_->odom_data_.q_.x();
      uav_action_msg.pose.orientation.y = data_->odom_data_.q_.y();
      uav_action_msg.pose.orientation.z = data_->odom_data_.q_.z();
      uav_action_msg.pose.orientation.w = data_->odom_data_.q_.w();
      
      uav_actions_pub_.publish(uav_action_msg);
      INFO_MSG_BLUE("[RC-FSM] Published refined position to /uav_actions: " 
                      << pos_refine[0] << ", " << pos_refine[1] << ", " << pos_refine[2]);
      // ========== 新增结束 ==========
      return true;
    }
...
void RCCtrlFSM::drawText() {
  ...
  marker.scale.z = 0.15;
  marker.color.a = 1.0;
  marker.color.r = (float)(data_->is_in_collision_traj_ ? 1.0 : 0.0);
  marker.color.g = (float)(data_->is_in_collision_traj_ ? 0.0 : 1.0);
  marker.color.b = 0.0;
  ...
  marker_rc_status.id = 1;
  marker_rc_status.lifetime = ros::Duration(0.5);
  ...
  marker_rc_status.color.r = (float)(data_->rc_data_.activate_rc_ctrl_mode_ ? 1.0 : 0.0);
  marker_rc_status.color.g = (float)(data_->rc_data_.activate_rc_ctrl_mode_ ? 0.0 : 1.0);
...
void RC_Data_t::process_ch10_trigger() {
  ...
  if (takeoff_land_flag){
    ...
    if (ch10_trigger_count_  == 1 && (rcv_stamp_ - ch10_trigger_last_time_ > ros::Duration(0.5))){
      ...
      ch10_trigger_count_      = 0;
      ch10_value_change_count_ = 0;
      INFO_MSG_GREEN("[RC-FSM]: Publish "<< (is_armed_ ? "LAND" : "TAKEOFF") <<" command");
  ...
  if (!takeoff_land_flag && ch10_trigger_count_  == 2
    ...
    INFO_MSG_GREEN("[RC-FSM]: " << (activate_rc_ctrl_mode_ ? "RC-CTRL ON! Be careful!" : "RC-CTRL OFF!"));
    INFO_MSG_GREEN("[RC-FSM]: ==============================");
    return;
  }
