...
namespace ego_planner
{
  void EGOReplanFSM::init(ros::NodeHandle &nh)
  {
    ...
    uav_actions_pub_    = nh.advertise<geometry_msgs::PoseStamped>("/uav_actions", 10);
      ...
      out_loop2:;
        if (new_goal_clear)
        {
          ...
          // ========== 新增：发布到 /uav_actions ==========
            ...
            uav_action_msg.pose.position.x = final_goal_[0];
            uav_action_msg.pose.position.y = final_goal_[1];
            uav_action_msg.pose.position.z = final_goal_[2];
            
            uav_action_msg.pose.orientation.x = 0;
            uav_action_msg.pose.orientation.y = 0;
            uav_action_msg.pose.orientation.z = 0;
            uav_action_msg.pose.orientation.w = 1; 
            
            uav_actions_pub_.publish(uav_action_msg);
            ROS_WARN("[RC-FSM] Published refined position to /uav_actions: (%.3f, %.3f %.3f)" ,
