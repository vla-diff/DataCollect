...
public:
    ...
    void callback(const typename T::ConstPtr& msg) {
        ...
        // std::cout << "time spent: " << (ros::Time::now() - t1).toSec() *1e3 << " ms" << std::endl;
    }
};
...
int main(int argc, char** argv) {
    ...
    TimeSyncMessageForwarder<sensor_msgs::CameraInfo>
                depth("/camera/color/info", "/camera/color/info_sync", 10);
    // TimeSyncMessageForwarder<nav_msgs::Odometry>
    //             odom("/unity_depth_odom0", "/unity_depth_odom0_sync", 10);
